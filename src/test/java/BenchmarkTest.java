...
public class BenchmarkTest {
    //static Log LOGGER = LogFactory.getLog(BenchmarkTest.class);
    static Logger log = Logger.getLogger("tesglog");
    static {
    ...
    private PriceToken generatePriceToken(int t, int i, int px){
        ...
        //sigma1 = sigma0.pow(issuerKey3.getSigningKey().getExponentsYi().innerProduct(RingElementVector.of(incentiveSystem.zp.valueOf((int) 100), incentiveSystem.zp.valueOf((int) 100), incentiveSystem.zp.valueOf((int) 100)))).op(sigma0.pow(issuerKey3.getSigningKey().getExponentX())).compute();
    ...
    public void runProtocol(TwoPartyProtocolInstance userInstance, TwoPartyProtocolInstance platformInstance) {
        ...
        do {
            ...
            try {
                ...
                // log.info("serialized message length: "+bos.toByteArray().length);
                if (isUsersTurn ) {
                    userComLength += bos.toByteArray().length;
                    //System.out.println("user round"+ bos.toByteArray().length);
                    //System.out.println(message);
                } else {
                    platformComLength += bos.toByteArray().length;
                    // System.out.println("platform round"+ platformComLength);
                }
                oos.flush();
            } catch (IOException e) {
                throw new RuntimeException(e);
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            edu.sydney.pisces.Benchmark benchmark = new edu.sydney.pisces.Benchmark();
...
//            log.info("User communication Length: "+ benchmark.userMessageLength );
//            log.info("platform communication Length: "+ benchmark.platformMessageLength );
